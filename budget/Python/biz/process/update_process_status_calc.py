...
def update_mysql(client, element_name, path, df_status, result_status, original_status, user,
    ...
    for _, row in df_status.iterrows():
        ...
        df_entity_subset = fun_query_dimension('Entity', expression, ['name'])
        ...
        if element_name == 'ApprovalProgressRecord':
            sql = (
                ...
                f"and department_id = '{department_id_single}' and {status_column} in ('{original_status}') "
        ...
        elif element_name == 'datatable_block_control_WS_cube':
            sql = (
                f"select datablock_seg_1_value,datablock_seg_2_value ,datablock_id  from ${{{element_name}}} "
                f"where datablock_seg_1_value in ('{entity_ids_str}') and datablock_seg_2_value = '{department_id_single}'"
            )
...
def update_status(p1, p2):
    ...
    # 1、修改审批记录表的审批状态
    ApprovalProgressRecord_sqls = update_mysql(client, 'ApprovalProgressRecord',
                             '/10_Approval/', df_status, result_status, original_status, user, status_column='result_status')
    # 执行sql语句
    ...
    # 3、修改财务模型权限表的审批状态
    blockid_sqls = update_mysql(client, 'datatable_block_control_WS_cube',
                                '/01_Cube/', df_status, result_status,
    ...
    control_ck = DataTableClickHouse("process_control_WS_cube")

    payload = DatatableDataUpdateDTO(
        setList={'process_status': result_status},
        elementName='process_control_WS_cube',
        path='/01_Cube',
        whereList={'datablock_id': datablock_id,
                   'process_seg_3_value': ['Y1'],
                   'process_seg_1_value ': [year_id_single],
                   'process_seg_2_value': ['Difference','Adj01','Adj02','Adj03','Adj04','Actual','Past','New','Budget','Combinaion','NoScenario']
                   }
    )
...
def main(p1, p2):
    ...
    try:
        ...
        if (result_01['updateSuccessCount'] >= 1) \
                and (result_02['updateSuccessCount'] >= 1) \
...
if __name__ == '__main__':
    ...
    # p1 = {
    p2 = {'original_status': ['Status01'],
          'result_status': 'Status02',
          'form_data': [
              {
                  'entity_id': 'PS32005_01',
                  'department_id': 'Operation',
                  'year_id': '2025'},
              {
                  'entity_id': 'PS32040_01',
                  'department_id': 'Technical',
                  'year_id': '2025'}
          ]}
