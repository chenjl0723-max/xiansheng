...
def convert_to_middle(df, year, Entity,value_col='data', result_col='data'):
    """
    小类折算中类公共函数
...
def material_processing(p1, p2, Entity,year, Version):
    ...
    # 5. 查询折算系数 ，计算种类单耗
    zhonglei_df = convert_to_middle(material_df, last_year, Entity,value_col='data', result_col='data')
...
def calc_y0_budget(p1, p2,Entity, year, Version):
    """
    ...
    # =========================================================
    # 5. 折算得到中类单耗 B
    df_b = convert_to_middle(df_a, year, Entity,value_col='A', result_col='B')
    ...
    fix_drop = (
        ...
        f"Account{{YW0304;YW0316}}->Scenario{{Budget}}->"
...
def calc_y0_budget_mq00(p1, p2, Entity,year, Version):
    ...
    jg_df.loc[jg_df['Material'] == 'MQ00', 'Material'] = 'Nomaterial'
    ...
    fix_drop = (
        ...
        f"Department{{Operation}}->Version{{Y1}}->Allocation{{Original}}->"
        f"Misc1{{Nomisc1}}->Misc2{{Nomisc2}}->"
        f"Account{{YW0401}}->Scenario{{Budget}}->"
        f"Measure{{M9901;M9902;M9903}}->Period{{Noperiod}}->"
        f"Tax{{Tax}}->Year{{{year}}}"
    )
    drop_df = cube.query(fix_drop, compact=False, pivot_dim='Measure')
    ...
    # 合并下降幅度（按中类）
    df_b = df_a.merge(
        drop_df[['Entity', 'Material','Account', 'drop_rate']],
        how='left',
        suffixes=('', '_y'))
    ...
    stjx_df = stjx_dt.select(
        columns=[ 'Entity', 'Middle', 'Sed_Stjx'],
        where=f"Year = '{last_year}' and Middle = 'Nomaterial'"
    )

    df_b = df_b.merge(stjx_df[['Entity', 'Sed_Stjx']],how='left',on=['Entity'])

    df_b['data'] = df_b.apply(
        lambda row: row['A'] if row['A'] <= row['Sed_Stjx']
                    else row['A'] * (1 - row['drop_rate']),
        axis=1
    )
    ...
    # =========================================================
    result = df_b[['Entity', 'Material', 'Year', 'Account', 'Period', 'Department',
...
def main(p1, p2):
    ...
    # 计算技改前后单耗
    material_processing(p1, p2, Entity, year, Version)

    # 先执行 MQ01/MQ03/MQ05
    if chem_sludge_materials:
        print(f"🔧 处理化学/污泥物料组: {chem_sludge_materials}")

        calc_y0_budget(p1, p2,Entity, year, Version)
    else:
        print("⏭️ 无化学/污泥物料，跳过")
