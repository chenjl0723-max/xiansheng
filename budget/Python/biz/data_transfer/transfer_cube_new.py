...
def query_sql():
    dt = DataTableMySQL('Account_td')
    df = dt.select(columns=['name'])
    return df
# 抽取cube数据
def extract_cube(account_map):
    dt = DataTableMySQL('cube_copy')
    result_dfs = []  # 存储所有月份的结果

    # 循环年份和月份
    for year in range(2020, 2027):  # 2020 到 2026
        for month in range(1, 13):  # 1 到 12 月
            ...
            where = "Year = '%s' and Period = '%s'" % (year, period)
            # cube_pov = f"Entity{{Base(#root,0)}}->Department{{Base(#root,0)}}->Year{{{year}}}->Period{{{period}}}->Account{{Base(#root,0)}}"

            # 查询 Cube 数据
            try:
                df = dt.select(where=where)
                print(f"查询 Year={year}, Period={period}, 原始数据行数: {len(df)}")
                # df = df[df['Account'].isin(account_map['name'])]
                ...
                # df_new = df_new.loc[df_new['Department'] == 'Technical']

                print(f"Year={year}, Period={period}, 处理后数据行数: {len(df)}")
                data_to_cube(df)
