...
# -*- coding:utf-8 -*-
"""
    描述：污水二期 检验化验费、其他成本、费用清单（Python）
...
class ITOA(object):
    ...
    def get_account_p2(self, p2):
        ...
        if p2["sheetName"] == "其他成本汇总表&检验化验费":
            p2["Account_wb1"] = "Base(PL010206,0);Base(PL010214,0);Base(PL010209,0);Base(PL010210,0);Base(PL010211,0);Base(PL010207,0);Base(PL010208,0);Base(PL010213,0);Base(PL010212,0);Base(PL010215,0);Base(PL010216,0);Base(PL010217,0);Base(PL010218,0)"
        del p2["sheetName"]
        del p2["sheetId"]
        print('方法1结束 ------')
        print('p2 =' + str(p2))
...
def main(p1, p2):
    ...
    # 需求2、新增计算，保存后运行
    # 二期新增 检验化验费、其他成本 含税金额  按月汇总至去年全年 2024
    ...
    # 新增需求：计算并转化 预算年 1-12 不含税-实际数 -> 含税
    ...
    audit = time.time()
    from budget.Python.biz.phaseII.newly.indicators_inspection import main as main_audit
    main_audit(p1, p2)
...
if __name__ == "__main__":
    try:
        ...
        pass
    para2 = {'elementName': 'Othercosts_and_Inspection',
             'folderId': 'DIRb6550dd20485',
             'sheetName': '其他成本汇总表&检验化验费',
             'sheetId': 'SHT7f54676534ca4a74b504b95ee5b21628',
             'Year_wb1': '2025',
