# installation_name
# 1 设备：新增设施拼接列（技改） factory_no+equip_seq_new+name_new = name
# 1 新增设备拼接列（技改）“名称”=水厂期数+设施序号+【设施标准名称】 location equip_seq_new、name_new = name ，例如1#1#鼓风机房
...
class Name_concatenation():
    def __init__(self,p1,p2):
        ...
        col = ['year','code','factory_no','name','name_new','equip_seq_new']
    ...
    def name_add(self,p1, p2):
        ...
        dim_factory_no = dim.query('Number{Descendant(factory_no,0)}', as_model=False, fields=["description_zh_cn"])
        dt_factory_no = pd.DataFrame(dim_factory_no)
        ...
        # 3.2 读取维度 Equipment
        dim = Dimension('Equipment', path='/02_Dimension/')
        # dim_Facility = dim.query('Equipment{Base(GZW,0);Base(CJ,0);Base(XT,0)}', as_model=False, fields=["aggweight", "description_zh_cn"])
        dim_Equipment = dim.query('Equipment{Children(GZW,0);Children(XT,0);Children(CJ,0)}', as_model=False,
                                 fields=["description_zh_cn"])
        dt_Equipment = pd.DataFrame(dim_Equipment)
        ...
        # 替换水厂期数 factory_no
        mapping = dict(zip(dt_factory_no['name'], dt_factory_no['description_zh_cn']))
        self.profile_df['factory_no'] = self.profile_df['factory_no'].map(mapping).fillna(
            self.profile_df['factory_no'])
        ...
        # 替换设备名称 name_new
        mapping = dict(zip(dt_Equipment['name'], dt_Equipment['description_zh_cn']))
        ...
        self.profile_df['name'] = self.profile_df['factory_no'].astype(str) + self.profile_df['equip_seq_new'].astype(str) + self.profile_df['name_new'].astype(str)
        print(self.profile_df)
        print(1)
