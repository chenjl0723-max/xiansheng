...
class Add_Entity:
    def __init__(self,entity):
        ...
        print('增量插入维度权限及审批记录表的项目编号：',self.df_entity)
        # 年份
        ...
        self.df_version = dim_.get_dim_attr(
            "Version", "Base(#root, 0)", ["name"], "/02_Dimension"
        )
    ...
    def init_block(self):
        ...
        """
        table = ck("datatable_block_control_BEWG")
        ...
        print("datatable_block_control_BEWG", i)
        # 存一下，后边的方法使用
        self.df_block = df_data

    def init_process(self):
        """
        增量初始化process_control_BEWG
        """
        table = ck("process_control_BEWG")
    ...
    def init_basic(self):
        ...
        if df_basic.empty:
            return
        # 获取所有basic_info表中的数据
        df_target = rdb_.select(["entity"], "basic_info", path="/05_Datatable/05_05_basic_info")
        ...
        msg = rdb_.insert_sql("basic_info", df_basic, path="/05_Datatable/05_05_basic_info")
    ...
    def init_progress(self):
        ...
        list_scenario = ["Actual", "Budget", "Forecast"]
        ...
        # 保存审批记录表
        msg = rdb_.insert_sql("ApprovalProgressRecord", df_pro, path="/10_Approval")
        return msg
...
if __name__ == "__main__":
    from common._debug import p1, p2
