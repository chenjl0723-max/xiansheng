    ...
    def calc_data(self, config_df):
        ...
        for form in form_list:
            ...
            for idx, query_df in config_select_df.iterrows():
                ...
                print(f"正在查询 calc={query_df['calc']}，fix={query_fix}")
                calc_df = cube_.query_cube(cube_name=self.cube, fix=query_fix)

                if calc_df.empty:
                    print(f"【空数据】Account={query_df['Account']}，Year={query_df['Year']}，"
                          f"Scenario={query_df['Scenario']}，Period={query_df['Period']}，"
                          f"Entity={query_df['Entity']}，Version={query_df['Version']}")
                    continue
                ...
                # 保留所需字段
                calc_df = calc_df[
                    ["Entity", "%s" % query_df["calc"]]
                ]

                # 拼接列
                if not calc_data_df.empty:
                    calc_data_df = pd.merge(calc_data_df, calc_df, on="Entity", how="outer")
            ...
            for idx, insert_df in config_insert_df.iterrows():
                ...
                # 关键：从 insert 配置行中取正确的 Period（而不是从查询数据继承）
                # temp_df["Period"] = insert_df["Period"]  # 例如 'Noperiod' 或 'TotalPeriod'
                # temp_df["Account"] = insert_df["Account"]

                # 构建最终插入数据
                df_insert = calc_data_df[["Entity", "data"]].copy()

                # 添加其他固定维度（从 insert 配置行取值）
                fixed_cols = [
                    "Scenario", "Measure", "Tax", "Version", "Year", "Account","Period",
                    "Department", "Material", "Allocation", "Misc1", "Misc2"
                ]
                for col in fixed_cols:
                    if col in insert_df and pd.notna(insert_df[col]):
                        df_insert[col] = insert_df[col]

                # 合并所有计算结果
                insert_res_df = pd.concat([insert_res_df, df_insert], ignore_index=True)
                print(1)
        # 数据存储
