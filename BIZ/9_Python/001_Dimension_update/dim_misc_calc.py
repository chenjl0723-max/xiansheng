...
def control_misc(df, p1):
    ...
    # rdb_.insert_sql(tbl="bewg_python_log", data=dt_log, path="/05_Datatable/05_11_Log/")
    return rsg
...
def deal_data(df, p1):
    ...
    dim_material = dim_.get_dim_attr(
        "Material", "Base(MQ,0)", dim_col, path="/2_Dimension/"
    )
    ...
    dim_entity = dim_.get_dim_attr(
        "Entity", "Base(1,0)", ["name", "parent_name"], path="/2_Dimension/"
    )
