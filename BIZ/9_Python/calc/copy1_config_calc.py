    ...
    def calc_data(self, config_df):
        ...
        for form in form_list:
            ...
            for idx, query_df in config_select_df.iterrows():
                ...
                # 保留所需字段
                calc_df = calc_df[
                    ["Entity", "Format","PM_Chars","%s" % query_df["calc"]]
                ]
            ...
            if not calc_data_df.empty:
                ...
                for idx, insert_df in config_insert_df.iterrows():
                    ...
                    elif insert_df['calc'] == 'A*10000/B':
                        calc_data_df["data"] = calc_data_df["A"] * 10000 / calc_data_df["B"]
                    calc_data_df['Account'] = insert_df['Account']
                    # 构建需要插入的数据
                    df_insert = calc_data_df[["Entity", "Account","PM_Chars", "data"]]
                    # df_insert["Period"] = insert_df["Period"]
                    df_index = insert_df[
                        [
                            "Period",
                            "Scenario",
