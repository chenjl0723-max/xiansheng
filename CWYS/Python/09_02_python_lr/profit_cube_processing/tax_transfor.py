...
def main_processing(p1, p2):
    ...
    # 获取预测数
    fix_forecast = "Year{%s}->Scenario{Forecast}->Version{%s}->Period{Base(TotalPeriod,0)}->Comprehensive{Tax}->" \
               "Entity_GL{%s}->Measure{Expenses}->Account_lirun{Base(PL600102,0);Base(PL60020202,0)}->"\
               "Misc1{nomisc1}->Misc2{nomisc2}->Commercial{Base(YT00,0)}" \
                % (last_Year,Version,Entity_GL)
    ...
    # 获取预算数
    fix_budget = "Year{%s}->Scenario{Budget}->Version{%s}->Period{Base(TotalPeriod,0)}->Comprehensive{Tax}->" \
               "Entity_GL{%s}->Measure{Expenses}->Account_lirun{Base(PL600102,0);Base(PL60020202,0)}->"\
               "Misc1{nomisc1}->Misc2{nomisc2}->Commercial{Base(YT00,0)}" \
...
def tax_rate_processing(df,Year,Version,Entity_GL,Scenario):
    ...
    df_no_rate = df_tax[df_tax['rate_data'].isna()].copy()
    if not df_no_rate.empty:
        if Scenario == 'Actual':
            df_no_rate['Comprehensive'] = 'Tax'
        elif Scenario in ['Budget', 'Forecast']:
            df_no_rate['Comprehensive'] = 'NoTax'
        # 保持原数据不变
        # df_no_rate = df_no_rate.drop(columns=['rate_data'])

    # 对于有税率的行，进行税率转换计算
    df_with_rate = df_tax[df_tax['rate_data'].notna()].copy()
    if not df_with_rate.empty:
        if Scenario == 'Actual':
            ...
            df_with_rate['data'] = df_with_rate['data'] / (1 + df_with_rate['rate_data'])
            df_with_rate['Comprehensive'] = 'NoTax'
...
def push_processing(df,cube,Year,Version,Entity_GL):
    ...
    del_fix1 = "Year{%s}->Scenario{Actual}->Version{%s}->Period{Base(TotalPeriod,0)}->Comprehensive{Tax}->" \
                   "Entity_GL{%s}->Account_lirun{Base(PL600102,0);Base(PL60020202,0)}"\
                    % (last_year,Version,Entity_GL)
    del_fix2 = "Year{%s}->Scenario{Forecast}->Version{%s}->Period{Base(TotalPeriod,0)}->Comprehensive{NoTax}->" \
                   "Entity_GL{%s}->Account_lirun{Base(PL600102,0);Base(PL60020202,0)}"\
                    % (last_year,Version,Entity_GL)
    del_fix3 = "Year{%s}->Scenario{Budget}->Version{%s}->Period{Base(TotalPeriod,0)}->Comprehensive{NoTax}->" \
                   "Entity_GL{%s}->Account_lirun{Base(PL600102,0);Base(PL60020202,0)}"\
                    % (Year,Version,Entity_GL)
...
def main(p1, p2):
    ...
    countSuccess = 0
    countError = 0
    countMsg = f"{start_time} 开始执行收入成本税率转换任务\n"

    try:
        main_processing(p1, p2)
        countSuccess = 1
        countAll = 1
        countMsg += f"{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())} 收入成本税率转换处理完成\n"
...
if __name__ == '__main__':
    para2 = {'Year_wb1':'2026','Version_wb1':'V1','Entity_GL_wb1':'Q92025001165'}
    main(para1, para2)
