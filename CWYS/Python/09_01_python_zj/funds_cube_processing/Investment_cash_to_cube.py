# -*- coding: utf-8 -*-
'''
@file    : Investment_cash_to_cube.py
@Desc    : 投资性现金流明细信息进cube
'''
...
def main_processing(p1, p2):
    ...
    fix_budget_adjb = "Year{%s}->Scenario{budget_adjb}->Version{%s}->Period{Base(TotalPeriod,0)}->Comprehensive{nocompr}->Counterparty{Base(sum,0)}->" \
               "Entity_FR{%s}->Measure{Expenses;Ext_Amt;IC_Amt}->Account_zijin{CF02020103;CF03070302;CF02020101;CF02020102;CF020203;CF03030101;CF03030102;CF03070201}->"\
    ...
    # 获取交易对手方信息填报表
    dt = DataTableMySQL('cp_info_invest')
    where = 'account in ("CF02020103","CF03070302","CF02020101","CF02020102","CF020203","CF03030101","CF03030102","CF03070201") and year = "%s" and entity = "%s"' % (
    Year, Entity_FR)

    if Entity_FR == 'Base(#root,0)':
        where = where = 'account in ("CF02020103","CF03070302","CF02020101","CF02020102","CF020203","CF03030101","CF03030102","CF03070201") and year = "%s"' % (
    Year)
...
def sum_actual_period_processing(p1,p2):
    ...
    fix_act_adjb = "Year{%s}->Scenario{actual_adjb}->Version{%s}->Period{Sep;10;11;12}->Comprehensive{nocompr}->Counterparty{Base(sum,0)}->" \
                      "Entity_FR{%s}->Measure{Expenses}->Account_zijin{CF02020103;CF03070302;CF02020101;CF02020102;CF020203;CF03030101;CF03030102;CF03070201}->" \
                      "Misc1{nomisc1}->Misc2{nomisc2}->Commercial{Base(YT00,0)}" \
...
def budget_forecast_nocp_processing(p1,p2):
    Year = p2['Year_wb1']
    last_Year = str(int(Year)-1)
    Version = p2['Version_wb1']
    Entity_FR = p2['Entity_FR_wb1']
    ...
    fix_forecast_adjb = "Year{%s}->Scenario{Forecast}->Version{%s}->Period{Base(TotalPeriod,0);Noperiod}->Comprehensive{nocompr}->Counterparty{sum}->" \
               "Entity_FR{%s}->Measure{Expenses}->Account_zijin{CF02020103;CF03070302;CF02020101;CF02020102;CF020203;CF03030101;CF03030102;CF03070201}->"\
               "Misc1{nomisc1}->Misc2{nomisc2}->Commercial{Base(YT00,0)}" \
                % (last_Year,Version,Entity_FR)
    df_forecast = cube.query(fix_forecast_adjb, compact=False)
    ...
    def_fix_forecast =  "Year{%s}->Scenario{Forecast}->Version{%s}->Period{Base(TotalPeriod,0);Noperiod}->Comprehensive{nocompr}->Counterparty{nocp}->" \
                      "Entity_FR{%s}->Measure{Expenses}->Account_zijin{CF02020103;CF03070302;CF02020101;CF02020102;CF020203;CF03030101;CF03030102;CF03070201}->" \
                      "Misc1{nomisc1}->Misc2{nomisc2}->Commercial{Base(YT00,0)}" \
                      % (last_Year, Version, Entity_FR)
...
def main(p1, p2):
    ...
    try:
        ...
        budget_forecast_nocp_processing(p1,p2)
...
if __name__ == '__main__':
    para2 = {'elementName': 'inter_co_tx_dtl',
             ...
             'Year_wb1': '2026',
             'Entity_FR_wb1': 'Y92024000770',
             'Version_wb1': 'V1',
             'Commercial_wb1': 'YT010101'}
  # 修改这里可测试不同年份
    main(para1, para2)